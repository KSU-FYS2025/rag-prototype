        ...
        if isinstance(data, dict) and data.get("action") == "commit":
            try:
                await websocket.send_json(
                    {
                        "status": "processing",
                        "message": f"Committing {len(self.received)} POIs...",
                    }
                )
                self.validate()
                res = self.send_to_db()
    ...
    def send_to_db(self) -> dict:
        ...
        for i, poi in enumerate(pois_to_process):
            if not poi.vector:
                ...
                text = (
                    f"The {poi.type} named '{poi.poiName}' (also known as {poi.name}) is a point of interest titled '{poi.title}'. "
        ...
        with get_db_gen() as db:
            ...
            res = db.upsert(collection_name="poi", data=data_to_insert)
        return res
